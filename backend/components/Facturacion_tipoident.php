...
{
    const MODULO='TIPO IDENTIFICACION';

    public function getTipoidentificacion($tipo,$array=true,$orderby,$limit,$all=true)
    {
        if ($all){
            $asiento= Tipoidentificacion::find()->where(["isDeleted"=>0])->all();
        }else{
            $asiento= Tipoidentificacion::find()->where(["isDeleted"=>0])->one();
        }
    }
 
    public function getSelect()
    {
        $clientes = Tipoidentificacion::find()->where(["isDeleted" => 0])->orderBy(["nombre" => SORT_ASC])->all();
        //var_dump($clientes);
        $clientesArray=array();
        $cont=0;
        foreach ($clientes as $key => $value) {
            if ($cont==0){ $clientesArray[$cont]["value"]="Seleccione un tipo de idenitificacion"; $clientesArray[$cont]["id"]=-1; $cont++; }
    ...
    public function Editar($data)
    {
        ...
        $result=false;
        if ($data):
            $model= Tipoidentificacion::find()->where(["id"=>$data['id']])->one();
            $model->nombre=$data['nombre'];
    ...
    public function Editarantecedentes($data)
    {
        ...
        $result=false;
        if ($data):
            $model= Tipoidentificacion::find()->where(["id"=>$data['idpaciente']])->one();
    ...
    public function Eliminar($id)
    {
        ...
        if ($id):
            //$data = $usuario;
            $data= Tipoidentificacion::find()->where(["id"=>$id])->one();
            $data->isDeleted=1;
            if ($data->save()) {
                $error=false;
