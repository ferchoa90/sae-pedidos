...
$(document).on("click", "#btn_save", function () {
    ...
    if (validarForm(obj)) {
        ...
        var obj = {
            nombre : $("#nombre").val(),
            titulo : $("#titslider").val(),
            titslider : $("#titulo").val(),
            textslider: $("#textslider").val(),
            textboton : $("#textboton").val(),
            linkboton : $("#lboton").val(),
            tag : $("#tag").val(),
...
function saveData(datos) {
    swal({
        ...
        function () {
            ...
            formData.append('contenido',  ( CKEDITOR.instances.editor.getData()));
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if (data.success) {
                        loading(1);
                        $.notify('Se ha generado una nueva página');
                        if (data.id)
                        {
        ...
        function () {
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if (data.success) {
                        loading(1);
                        $.notify('Se ha actualizado la página');
                        if (data.id)
                        {
                            window.location.href = "view?id=" + data.id;
                        }
